// Package synccards adapts the local cards repository to syncer.Repo.
package synccards
...
func NewRepo(cards *repository.CardsRepo, state *repository.SyncStateRepo) *Repo {
	return &Repo{cards: cards, state: state}
}

var _ syncer.Repo = (*Repo)(nil)

func (r *Repo) ListDirty(ctx context.Context) ([]syncer.Row, error) {
	rows, err := r.cards.ListDirty(ctx)
	if err != nil {
		return nil, err
	}
...
func (r *Repo) Get(ctx context.Context, id string) (syncer.Row, bool, error) {
	row, ok, err := r.cards.GetRow(ctx, id)
	if err != nil || !ok {
