...
func TestRepoDelegates(t *testing.T) {
	...
	db := newDB(t)
	cards := repository.NewEntryRepo(db, repository.TableCard)
	r := NewRepo(cards, repository.NewSyncStateRepo(db))

	var _ syncer.Repo = r

	if err := r.Upsert(ctx, "c1", []byte("hello"), 3); err != nil {
		t.Fatalf("upsert: %v", err)
	}
	row, ok, err := r.Get(ctx, "c1")
	if err != nil || !ok {
	...
	created, err := cards.Create(ctx, []byte("local"))
	if err != nil {
	...
	if err = r.MarkConflict(ctx, "c1", []byte("srv"), 9); err != nil {
		t.Fatalf("mark conflict: %v", err)
	}

	if err = r.HardDelete(ctx, "c1"); err != nil {
		t.Fatalf("hard delete: %v", err)
	}
	if _, ok, _ := r.Get(ctx, "c1"); ok {
