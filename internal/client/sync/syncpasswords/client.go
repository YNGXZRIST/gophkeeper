...
type Client struct {
	c passwordv1.PasswordServiceClient
}

func NewClient(c passwordv1.PasswordServiceClient) *Client {
	return &Client{c: c}
}
...
func (a *Client) Create(ctx context.Context, id string, data []byte) (int64, error) {
	...
	return resp.GetPassword().GetVersion(), nil
}
