// Package syncpasswords adapts the local passwords repository to syncer.Repo.
...
func (r *Repo) ListDirty(ctx context.Context) ([]syncer.Row, error) {
	rows, err := r.passwords.ListDirty(ctx)
	if err != nil {
		return nil, err
	}
...
func (r *Repo) Get(ctx context.Context, id string) (syncer.Row, bool, error) {
	row, ok, err := r.passwords.GetRow(ctx, id)
	if err != nil || !ok {
