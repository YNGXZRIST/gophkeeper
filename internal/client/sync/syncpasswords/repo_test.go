...
func TestRepoDelegates(t *testing.T) {
	...
	db := newDB(t)
	passwords := repository.NewEntryRepo(db, repository.TablePassword)
	r := NewRepo(passwords, repository.NewSyncStateRepo(db))

	var _ syncer.Repo = r

	if err := r.Upsert(ctx, "p1", []byte("hello"), 3); err != nil {
		t.Fatalf("upsert: %v", err)
	}
	row, ok, err := r.Get(ctx, "p1")
	if err != nil || !ok {
	...
	created, err := passwords.Create(ctx, []byte("local"))
	if err != nil {
