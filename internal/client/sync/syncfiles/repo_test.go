...
func TestRepoDelegatesMetaMapping(t *testing.T) {
	ctx := context.Background()
	db := newDB(t)
	files := repository.NewFilesRepo(db)
	r := NewRepo(files, repository.NewSyncStateRepo(db))

	var _ syncer.Repo = r

	if err := r.Upsert(ctx, "f1", []byte("meta-bytes"), 3); err != nil {
		t.Fatalf("upsert: %v", err)
	}
	row, ok, err := r.Get(ctx, "f1")
	if err != nil || !ok {
		t.Fatalf("get: ok=%v err=%v", ok, err)
	}
	if string(row.Data) != "meta-bytes" || row.Version != 3 || row.BaseVersion != 3 || row.Dirty {
		t.Fatalf("row = %+v", row)
	}
	...
	if len(dirty) != 1 || dirty[0].ID != "f2" || !dirty[0].Dirty || string(dirty[0].Data) != "m2-updated" {
