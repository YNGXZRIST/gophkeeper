// Package passwordconflict resolves password sync conflicts and shows sync errors.
...
const label = "Password conflicts"
...
type item struct {
	id     string
	local  clientmodel.PasswordData
	server clientmodel.PasswordData
}
...
func line(d clientmodel.PasswordData) string {
	login := d.Login
	if login == "" {
		login = "—"
	}
	return login + " · " + mask(d.Password)
}
...
func decode(v *vault.Vault, blob []byte) clientmodel.PasswordData {
	var d clientmodel.PasswordData
