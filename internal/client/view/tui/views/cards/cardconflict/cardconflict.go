// Package cardconflict resolves card sync conflicts and shows sync errors.
...
type item struct {
	id     string
	local  clientmodel.CardData
	server clientmodel.CardData
}
...
func line(d clientmodel.CardData) string {
	parts := make([]string, 0, 2)
	if n := maskNumber(d.Number); n != "" {
		parts = append(parts, n)
	}
	if d.Holder != "" {
		parts = append(parts, d.Holder)
	}
	if len(parts) == 0 {
		return "—"
	}
...
func decode(v *vault.Vault, blob []byte) clientmodel.CardData {
	var d clientmodel.CardData
